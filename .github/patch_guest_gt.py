...
(function(){
  ...
  function openPicker(input,monthHint){pickerTarget=input; const d=dateObj(input.value)||dateObj(monthHint)||new Date(); pickerMonth=new Date(d.getFullYear(),d.getMonth(),1); renderPicker();}
  function closePicker(){if(picker){picker.remove();picker=null;pickerTarget=null}}
  function renderPicker(){
    closePicker(); if(!pickerTarget)return;
    ...
    for(let day=1;day<=count;day++){const d=new Date(pickerMonth.getFullYear(),pickerMonth.getMonth(),day),e=document.createElement('button');e.type='button';e.textContent=day;const val=`${pad(day)}.${pad(d.getMonth()+1)}.${d.getFullYear()}`; if((min&&d<min)||(max&&d>max)){e.disabled=true;e.className='disabled'} if(fmtDate(pickerTarget.value)===val)e.className='selected'; e.onclick=()=>{pickerTarget.value=val; pickerTarget.dispatchEvent(new Event('input',{bubbles:true})); closePicker();}; days.append(e)}
    picker.append(days);
  ...
  function globalDateChanged(id){
    ...
    } else if(id==='checkout') $('nights').value=calc(ci,co);
